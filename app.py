...
class ResilientClient(httpx.AsyncClient):
    """Bounded retries for read-only provider requests; shared per-host breaker."""
    failures = {}
    opened_until = {}
    ...
    async def request(self, method, url, **kwargs):
        ...
        now = time.monotonic()
        if self.opened_until.get(host, 0) > now:
            raise RuntimeError(f"Provider circuit open: {host}")
            ...
            try:
                ...
                if response.status_code == 429:
                    try:
                        ...
                        delay = 30
                    delay = max(1, min(delay if math.isfinite(delay) else 30, 300))
                ...
                if self.failures[host] >= 5:
                    self.opened_until[host] = time.monotonic() + 30
                if attempt == 2 or self.opened_until.get(host, 0) > time.monotonic():
                    raise RuntimeError(f"Provider unavailable: {host} ({type(error).__name__})") from None
...
async def lifespan(application):
    if len(ADMIN_KEY) < 32 or any(ch.isspace() for ch in ADMIN_KEY):
        raise RuntimeError("NOVA_ADMIN_KEY must contain at least 32 characters without whitespace")
    await startup()
    try:
        yield
    finally:
...
app = FastAPI(title="NOVA V7 Paper Trading", version=APP_VERSION, lifespan=lifespan)
        ...
        try:
            ...
            for mint,p in best.items():
                ...
                pairs.append({
                    ...
                    "perp_eligible": False,
                    "perp_market": None,
                    "data_source":"DEXSCREENER","received_at":time.time(),
...
def partial_sell(p, c, fraction):
    if not valid_market_mark(c) or not math.isfinite(fraction) or fraction <= 0:
        raise ValueError("Invalid partial exit price or fraction")
...
def close_position(p,c,reason):
    with SessionLocal() as check_session:
        current = check_session.get(Position, p.id)
        if current is None:
            return
        p = current
    if not valid_market_mark(c):
        raise ValueError("Cannot simulate an exit without a fresh positive price")
    side=strategy_side(p.strategy)
    leverage=strategy_leverage(p.strategy)
    mid_price=nz(c.get("price"))
    execution_notional=p.remaining_cost*leverage
...
def manage_positions():
    ...
    for p in pos:
        c=cands.get(p.mint)
        if not c or not valid_market_mark(c):
...
async def startup():
    ...
    app.state.background_tasks = [
        ...
        (engine_loop, position_watch_loop, sniper_scan_loop, pumpportal_realtime_loop)
    ]
