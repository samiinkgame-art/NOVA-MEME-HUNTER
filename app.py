...
from nova_core.http import ResilientClient
from nova_core.auth import validate_key, RateLimiter
from nova_core.money import dec
from nova_core.data import normalize_pair
from nova_core.schema import IntegrityBase, Intent, ExactPosition, Ledger, TokenObservation
...
RELEASE_LABEL = "NOVA Integrated Paper · 1.4.0 RC1"
...
BINANCE_SPOT_ENABLED = os.getenv("NOVA_BINANCE_SPOT_ENABLED", "false").lower() in ("1","true","yes","on")
BINANCE_FUTURES_ENABLED = False  # MEME HUNTER: perps/major trader moved to separate bot
DEX_MULTI_CHAIN_ENABLED = os.getenv("NOVA_DEX_MULTI_CHAIN_ENABLED", "false").lower() in ("1","true","yes","on")
...
PROFIT_CYCLE_ENABLED=os.getenv("NOVA_PROFIT_CYCLE","false").lower() in ("1","true","yes","on")
...
MICRO_PROFIT_ENABLED=os.getenv("NOVA_MICRO_PROFIT","false").lower() in ("1","true","yes","on")
...
_unit_session = ContextVar("nova_unit_session", default=None)
_account_lock = threading.RLock()

class _BorrowedSession:
    """Nested legacy helpers share the outer atomic transaction."""
...
def SessionLocal():
    session = _unit_session.get()
    return _BorrowedSession(session) if session is not None else _SessionFactory()

def account_transaction(function):
    @wraps(function)
    def wrapped(*args, **kwargs):
        with _account_lock:
            if _unit_session.get() is not None:
                return function(*args, **kwargs)
            with _SessionFactory.begin() as session:
                if engine.dialect.name == "sqlite":
                    session.connection().exec_driver_sql("BEGIN IMMEDIATE")
                elif engine.dialect.name == "postgresql":
                    session.execute(text("SELECT pg_advisory_xact_lock(726684014)"))
                else:
                    raise RuntimeError("Only SQLite and PostgreSQL are supported")
                token = _unit_session.set(session)
                try:
                    return function(*args, **kwargs)
                finally:
                    _unit_session.reset(token)
    return wrapped
...
DEFAULTS = {
    ...
    "selective_entry_router_enabled": "false",
...
async def lifespan(application):
    validate_key(ADMIN_KEY)
    await startup()
    try:
        yield
    finally:
        tasks = application.state.background_tasks
        for task in tasks:
            task.cancel()
        await asyncio.gather(*tasks, return_exceptions=True)

app = FastAPI(title="NOVA Integrated Paper Trading", version=APP_VERSION, lifespan=lifespan)
_request_limiter = RateLimiter()

@app.middleware("http")
async def protect_api(request: Request, call_next):
    if request.method != "OPTIONS" and request.url.path.startswith("/api/"):
        if not _request_limiter.allow(request.client.host if request.client else "unknown"):
            return JSONResponse({"detail":"Rate limit exceeded"},status_code=429,headers={"Retry-After":"60"})
        if not ADMIN_KEY_CONFIGURED and request.method != "GET":
            return JSONResponse({"detail":"Configure a strong NOVA_ADMIN_KEY to control paper trading"},status_code=403)
        try:
    ...
    response.headers["X-NOVA-Version"] = APP_VERSION
    return response
...
def auth(x_nova_key: Optional[str]):
    if not ADMIN_KEY_CONFIGURED:
        return  # Keyless deployment is read-only scanner; middleware blocks all mutation.
    candidate = normalize_admin_key(x_nova_key)
    if not candidate or not hmac.compare_digest(candidate.encode(), ADMIN_KEY.encode()):
        raise HTTPException(401, "Invalid NOVA admin key")

def nz(v, d=0.0):
    try:
        result = float(v or d)
        return result if math.isfinite(result) else float(d)
    except: return d


def operating_mode():
    mode=str(getv("operating_mode","PAPER")).upper()
    return "SCANNER" if not ADMIN_KEY_CONFIGURED or mode == "SCANNER" else mode if mode in ("PAPER","SHADOW") else "PAPER"
...
async def scan_token_security(client,c,force=False):
    ...
    try:
        ...
        if not isinstance(parsed, dict) or parsed.get("type") != "mint":
            raise ValueError("RPC did not return a parsed mint")
        supply_raw=float(info.get("supply") or 0)
        if not math.isfinite(supply_raw) or supply_raw <= 0:
            raise ValueError("Mint supply unavailable")
        if "mintAuthority" not in info or "freezeAuthority" not in info:raise ValueError("Authority fields unavailable")
        ...
        if owner == "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb":
            hard=True;flags.append("Token-2022 extensions require a dedicated audit")
        if mint_auth:
            score-=18;flags.append("mint authority active");hard=True
        if freeze_auth:
        ...
        result={
            ...
            "coverage":"PARTIAL","rug_score":None,"developer_score":None,"checked_at":datetime.now(timezone.utc).isoformat(),
            "note":"Partial mint scan only, not a safety certificate. Token accounts are not holder identities; LP/treasury accounts may dominate concentration."
        }
...
def security_coverage():
    ...
    return {
        ...
        "coverage_pct":(len(known)/len(top)*100) if top else 0
    }
...
def source_health():
    ...
    # Disabled major/perp engines are not dependencies of the Solana scanner.
    return {
...
async def discover(client):
    ...
    addresses=list(dict.fromkeys(manual+[x["mint"] for x in public_discovery.recent(30)]+addresses))
    ...
    for k in range(0,len(addresses),30):
        ...
        try:
            ...
            for mint,p in best.items():
                ...
                if price<=0: continue
                try: observation=normalize_pair(p,time.time())
                except (ValueError,TypeError):continue
                first_seen=integrity.observation(observation)
                ...
                pairs.append({
                    ...
                    "perp_eligible": False,
                    "perp_market": None,
                    "data_source":"DEXSCREENER","received_at":observation.received_at,
                    "first_seen":first_seen,"pair_created_at":observation.pair_created_at,"pair_address":observation.pool,
                    "volume_h1":observation.volume1h,"observation":observation.dict(),
...
def _spot_candidate_from_dex_pair(p, boost=0.0, force_chain=None):
    ...
    return {
        ...
        "asset_class":_asset_class(symbol,"SPOT"),"data_source":"DEXSCREENER_MULTI" if chain!="solana" else "DEXSCREENER","received_at":time.time(),
...
def _binance_spot_candidate(info,ticker):
    ...
    return {
        ...
        "asset_class":_asset_class(base_asset,"SPOT"),"data_source":"BINANCE_SPOT","received_at":time.time(),"cex_listed":True,"depth_is_proxy":True,
    ...
    with SessionLocal() as s:
        for p in s.scalars(select(Position)).all():
            ...
            if b("execution_simulator_enabled") and c and p.remaining_cost>0:
                ...
                fee=gross*est.get("fee_bps",0)/10000.0
...
def execution_reference_depth(c,strategy):
    ...
    return max(0.0,nz(c.get("liquidity")))
...
def valid_execution_liquidity(c):
    return valid_market_mark(c) and nz(c.get("liquidity"))>0 and not c.get("price_is_proxy") and c.get("data_source")!="BINANCE_SPOT"

def execution_quality_gate(c,strategy,notional_usd):
    if not valid_execution_liquidity(c):return False,"paper liquidity unavailable",{}
    if notional_usd>nz(c.get("liquidity"))*.01:return False,"paper participation limit",{}
...
def planned_collateral(c,strategy):
    ...
    cost=execution_cost_estimate(c,strategy,max(5,m["equity"]*f("max_position_pct")/100))["all_in_bps"]*2/10000
    collateral=risk_budget/max((effective_stop/100)*leverage+cost,0.001)
...
def valid_market_mark(c):
    try:
        price = float(c.get("price", 0))
        age = time.time() - float(c.get("received_at", 0))
        return math.isfinite(price) and price > 0 and 0 <= age <= f("max_data_age_sec")
    except (TypeError, ValueError):
        return False

def gate(c,strategy=None):
    if not valid_market_mark(c): return False,"invalid or stale token price"
    if c.get("price_is_proxy") or c.get("data_source") == "BINANCE_SPOT":
        return False,"executable market depth unavailable"
    if c.get("perp_eligible"): return False,"perpetual execution not implemented in Meme Hunter"
    if (c.get("security") or {}).get("hard_block"):
        return False,"token security hard block"
    if b("killed"): return False,"kill switch"
    if operating_mode()=="SCANNER": return False,"scanner mode cannot enter trades"
    if getv("recovery_ok","false")!="true": return False,"recovery reconciliation required"
    if c.get("observation") and not c["observation"].get("complete"): return False,"incomplete observation coverage"
    period=integrity.periods()
    if period["reasons"]:return False,period["reasons"][0]
    if time.time()<float(getv("cooldown:"+str(c.get("mint")),"0")):return False,"persistent token cooldown"
    if time.time()<float(getv("loss_pause_until","0")):return False,"persistent loss pause"
...
def open_position(c,strategy):
    ...
    integrity.cash_add(-(dec(collateral)+dec(fee)))
    now=datetime.now(timezone.utc)
    position_id=None
    with SessionLocal() as s:
        pos=Position(
            id=integrity.next_position_id(),
        ...
        s.add(pos);s.flush()
        position_id=pos.id
        integrity.entry(pos,collateral,fee,fill_price,c["received_at"])
...
@account_transaction
def partial_sell(p, c, fraction, request_id=None):
    return integrity.exit(p,c,fraction,final=False,event_key=request_id)

@account_transaction
def close_position(p,c,reason):
    result=integrity.exit(p,c,final=True)
    if result is None:return
    current,net,total=result
    closed=datetime.now(timezone.utc)
    with SessionLocal() as s:
        pf=s.scalar(select(PositionFeature).where(PositionFeature.position_id==current.id))
        tr=Trade(mint=current.mint,symbol=current.symbol,strategy=current.strategy,pnl=total,
            pnl_pct=total/max(current.initial_notional,1e-12)*100,reason=reason,opened_at=current.opened_at,closed_at=closed)
        s.add(tr);s.flush()
        if pf:
            s.add(TradeFeature(trade_id=tr.id,strategy=current.strategy,entry_score=pf.entry_score,market_risk=pf.market_risk,
                direction_edge=pf.direction_edge,funding_rate=pf.funding_rate,open_interest_usd=pf.open_interest_usd,
                volatility_regime=pf.volatility_regime,pnl=total,pnl_pct=tr.pnl_pct,closed_at=closed))
            s.delete(pf)
        s.delete(current);s.flush()
    until=time.time()+i("cooldown_minutes")*60
    setv("cooldown:"+current.mint,str(until));runtime["cooldowns"][current.mint]=until
    runtime["realtime_exit_refs"].pop(current.mint,None)
    record_event("INFO","POSITION_CLOSED",f"{current.symbol} closed: {reason}",{"pnl":total})
    maybe_pause_strategy(current.strategy)
    if consecutive_losses()>=min(i("max_consecutive_losses"),i("governor_loss_streak_limit")):
        until=time.time()+max(i("loss_pause_minutes"),i("governor_loss_streak_pause_minutes"))*60
        setv("loss_pause_until",str(until));runtime["pause_until"]=datetime.fromtimestamp(until,timezone.utc)
...
def expected_exit_total_pct(p,c):
    mid=nz(c.get("price"))
    if mid<=0:return -100.0
    est=execution_cost_estimate(c,p.strategy,max(0,p.remaining_cost/max(p.entry_price,1e-12)*mid))
    fill=simulated_fill_price(mid,strategy_side(p.strategy),"MARK_EXIT",est["adverse_bps"])
    gross=p.remaining_cost/max(p.entry_price,1e-12)*fill
    net=gross*(1-est["fee_bps"]/10000)
    return (p.locked_pnl+net-p.remaining_cost)/max(p.initial_notional,1e-12)*100
...
def manage_positions():
    ...
    for p in pos:
        c=cands.get(p.mint)
        if not c or not valid_execution_liquidity(c):
        ...
        with SessionLocal() as s:
            ...
            if reason is None:
                ...
                if net_ret>=tp1[0] and not obj.tp1:
                    partial_sell(obj,c,tp1[1],request_id=f"tp1:{obj.id}");obj.tp1=True
                if net_ret>=tp2[0] and not obj.tp2:
                    partial_sell(obj,c,tp2[1],request_id=f"tp2:{obj.id}");obj.tp2=True
                if net_ret>=tp3[0] and not obj.tp3:
                    if obj.strategy=="SNIPER_LONG":
                        reason="SNIPER_TAKE_PROFIT"
                    else:
                        partial_sell(obj,c,tp3[1],request_id=f"tp3:{obj.id}");obj.tp3=True
...
def launch_gate(m):
    return False,"launch discovery only: executable price and depth not verified"
...
def launch_candidate(m,price=1.0):
    ...
    return {
        ...
        "perp_eligible":False,
        "data_source":"PUMPPORTAL_LAUNCH","received_at":time.time(),"price_is_proxy":True,
...
async def realtime_exit_check(mint,m):
    ...
    try:
        ...
        if now-runtime["realtime_exit_last_eval"].get(rest_key,0)>=rest_gap:
            ...
            async with ResilientClient(headers={"User-Agent":"NOVA-Realtime-Exit/7.0.0"}) as client:
...
async def evaluate_pulse_mint(mint,m):
    ...
    try:
        ...
        async with ResilientClient(headers={"User-Agent":"NOVA-Pulse-Sniper/7.0.0"}) as client:
...
async def _cost_balance():
    ...
    try:
        ...
        async with ResilientClient(timeout=8) as c:r=await c.post(SOLANA_RPC_URL,json=q)
...
async def pumpportal_realtime_loop():
    raise RuntimeError("Metered stream is not integrated; use the public creation stream")
...
def merge_position_updates(updates):
    ...
    for fresh in updates:
        ...
        if nz(fresh.get("received_at"))<=nz(old.get("received_at")):continue
...
async def sniper_scan_loop():
    ...
    async with ResilientClient(headers={"User-Agent":"NOVA-Meme-Hunter-Sniper/1.1.0"}) as client:
        while True:
            try:
                if b("sniper_enabled"):
                    addresses=[];boosts={}
...
async def position_watch_loop():
    ...
    async with ResilientClient(headers={"User-Agent":"NOVA-Meme-Hunter-Watcher/1.1.0"}) as client:
        while True:
            try:
                with SessionLocal() as s:
...
async def engine_loop():
    ...
    async with ResilientClient(headers={"User-Agent":"NOVA-Meme-Hunter/1.1.0"}) as client:
...
async def startup():
    integrity.recover()
    # Meme Hunter profile: legacy strategies retained; LIVE never enabled.
    ...
    # Restart never silently resumes entries; exits remain managed after fresh quotes.
    setv("bot_enabled","false")
    setv("launch_sniper_enabled","false")
    setv("execution_simulator_enabled","true")
    setv("no_martingale","true")
    if not ADMIN_KEY_CONFIGURED:setv("operating_mode","SCANNER")
    app.state.background_tasks = [
        asyncio.create_task(fn()) for fn in
        (engine_loop, position_watch_loop, sniper_scan_loop)
    ]

    if os.getenv("NOVA_PUBLIC_LAUNCH_STREAM","true").lower()=="true":
        app.state.background_tasks.append(asyncio.create_task(public_discovery.run()))
...
def capabilities():
    """Stable machine-readable feature contract for the upgraded V7 dashboard."""
    return {
        "version":APP_VERSION,
        "release_label":RELEASE_LABEL,
        "live_execution_locked":LIVE_EXECUTION_LOCKED,
        "paper_shadow_modes":["PAPER","SHADOW"],
        "universe":["DEX_MULTI_CHAIN","BINANCE_SPOT","LAUNCHES","MEME_ALTCOINS"],
        "intelligence":["APEX_SCORE","PRE_MOVE","ANTI_CHASE","LAUNCH_INTELLIGENCE","EDGE_GOVERNOR"],
        "risk":["DAILY_LOSS_GUARD","OPEN_RISK_CAP","TAIL_RISK","DE_RISK","NO_MARTINGALE"],
        "execution":["PAPER_SLIPPAGE","PAPER_FEES","PARTIAL_EXITS","EVENT_DRIVEN_EXITS"],
...
async def compat_paper_open(request: Request):
    ...
    def action():
        ...
        if not c:raise HTTPException(404,"Token not in scanner")
        ...
        pos=open_position_for_mint(mint)
        return {"id":str(pos.id),"mint":mint,"mode":"paper"}
    return integrity.request(request_id,{"action":"open","mint":mint,"acknowledgement":body.get("acknowledgement")},action)
...
async def compat_paper_close(request: Request):
    ...
    def action():
        with SessionLocal() as s:
            ...
            c=next((dict(x) for x in runtime['candidates'] if x.get('mint')==p.mint),None)
            if not c:raise HTTPException(409,"Fresh matching market data required")
            try:close_position(p,c,"MANUAL_EXIT")
            except ValueError as e:raise HTTPException(409,str(e))
            return {'ok':True,'position_id':str(position_id)}
    return integrity.request('close-position:'+str(position_id),{'action':'close','position_id':position_id},action)
...
async def security_rescan(mint:str, x_nova_key:Optional[str]=Header(None, alias="X-NOVA-Key")):
    ...
    async with ResilientClient(headers={"User-Agent":"NOVA-Trader-FreeLite/7.0.0"}) as client:
...
def set_engine_control(engine_name:str,state:str,x_nova_key:Optional[str]=Header(None, alias="X-NOVA-Key")):
    ...
    if enabled and name in ("launch","perp_long","perp_short"):
        raise HTTPException(409,"This engine has no verified executable price/depth adapter")
...
def set_mode(mode:str, x_nova_key:Optional[str]=Header(None, alias="X-NOVA-Key")):
    ...
    if requested not in ("SCANNER","PAPER","SHADOW"):
        raise HTTPException(400,"Mode must be SCANNER, PAPER or SHADOW")
...
def control(action:str, x_nova_key:Optional[str]=Header(None, alias="X-NOVA-Key")):
    auth(x_nova_key)
    if action=="start":
        if b("killed"):raise HTTPException(409,"Release kill switch explicitly before starting")
        if operating_mode()=="SCANNER":raise HTTPException(409,"Choose PAPER mode before starting entries")
        if not integrity.reconcile()["ok"]:raise HTTPException(409,"Ledger reconciliation failed")
        setv("bot_enabled","true")
        ...
        raise HTTPException(409,"Account reset is disabled; use a new backed-up database for a new experiment")
        # Historical reset code retained but unreachable:
        with SessionLocal() as s:
...
def settings(data:SettingsIn, x_nova_key:Optional[str]=Header(None, alias="X-NOVA-Key")):
    ...
    for name, value in vals.items():
        if isinstance(value, (int, float)) and not isinstance(value, bool) and value < 0:
            raise HTTPException(400, name + " must not be negative")
    caps = {"max_position_pct":(0.1,10),"max_total_exposure_pct":(1,35),
            "daily_loss_limit_pct":(0.1,5),"drawdown_hard_cut_pct":(0.1,10),
            "max_data_age_sec":(5,90),"max_execution_cost_bps":(1,120)}
    for name, (low, high) in caps.items():
        if name in vals and not low <= vals[name] <= high:
            raise HTTPException(400, f"{name} must be {low}..{high}")
    for name in ("execution_simulator_enabled", "no_martingale"):
        if name in vals and not vals[name]:
            raise HTTPException(400, name + " is mandatory")
    for name in ("launch_sniper_enabled", "perp_long_engine_enabled", "perp_short_engine_enabled"):
        if vals.get(name):
            raise HTTPException(409, name + " execution is unavailable")
    if "weekly_loss_limit_pct" in vals and not (0.1<=vals["weekly_loss_limit_pct"]<=10):
        raise HTTPException(400,"weekly_loss_limit_pct must be 0.1..10")
...
# Explicit dependencies: integrity and mobile modules never import app recursively.
import sys as _sys
from nova_core.integrity import Integrity
from nova_core.api import MobileAPI
from nova_core.discovery import PublicDiscovery
integrity=Integrity(_sys.modules[__name__])
mobile=MobileAPI(_sys.modules[__name__])
public_discovery=PublicDiscovery(_sys.modules[__name__])
mobile.install()
integrity.recover()
