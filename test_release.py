...
os.environ['NOVA_PUBLIC_LAUNCH_STREAM']='false'
import pytest
import httpx
...
def clean_account():
    with nova._SessionFactory.begin() as session:
        for table in reversed(nova.IntegrityBase.metadata.sorted_tables):
            session.execute(table.delete())
        for table in reversed(nova.Base.metadata.sorted_tables):
            session.execute(table.delete())
    nova.init_defaults()
    nova.integrity.recover()
    nova._request_limiter.items.clear()
    ...
    nova.runtime['loop_alive'] = False
    nova.runtime['last_successful_loop'] = None
...
def test_health_and_authenticated_dashboard():
    assert client.get('/health').json()['version'] == '1.4.0-rc1'
...
def test_dashboard_escapes_external_fields_and_labels_paper():
    ...
    if not root.exists():pytest.skip('Frontend lives in separate repository; run complete ZIP to test it')
    html=(root/'index.html').read_text();js=(root/'app.js').read_text()
    assert 'type="password"' in html
    assert 'esc(t.symbol)' in js and 'esc(JSON.stringify' in js
    assert "localStorage.setItem('nova_backend_origin',base)" in js
    assert 'localStorage.setItem(\'key' not in js
